...
/**
 * Calls highlight to begin parsing and tagging the document
 * @param  {int} lat latitude of location user entered
 * @param  {int} lon longitude of location user entered
 */
...
function highlight(article_content) {
  ...
  var myAreaRegexp = /([1-9](?:\d{0,2})(?:,\d{3})*(?:\.\d*[1-9])?|0?\.\d*[1-9]|0)[\s-]*((?:acres?|sq\.?[\s-]mi\b|square[\s-]miles?|sq\.?[\s-]mi|square[\s-]mi|sq\.?[\s-]yd|sq\.?[\s-]yards?|square[\s-]yards?|square[\s-]yds?|sq\.?[\s-]meters?|square[\s-]meters?|square[\s-]metres?|square[\s-]ft|sq\.?[\s-]feet|sq\.?[\s-]foot|sq\.?[\s-]ft|square[\s-]feet|square[\s-]foot|ft<sup>2<\/sup>|m<sup>2<\/sup>|km\<sup>2<\/sup>|km2|mi2|mi<sup>2<\/sup>))/gi;
  ...
  var myCountryRegexp = /\b(Andorra|United Arab Emirates|Afghanistan|Antigua and Barbuda|Anguilla|Albania|American Samoa|Austria|Aruba|Bosnia|Barbados|Belgium|Burkina Faso|Bulgaria|Bahrain|Burundi|Benin|Bermuda|Bolivia|Bhutan|Bouvet Island|Botswana|Belarus|Belize|Cocos Islands|Central African Republic|Congo|Switzerland|Cook Islands|Cameroon|Colombia|Costa Rica|Christmas Island|Cyprus|Czech Republic|Djibouti|Dominica|Dominican Republic|Algeria|Egypt|Western Sahara|Ethiopia|Micronesia|Gabon|Grenada|Georgia|French Guiana|Ghana|Gibraltar|Gambia|Guinea|Guadeloupe|Guatemala|Guam|Guinea-Bissau|Guyana|Heard Island and McDonald Islands|Honduras|Hungary|Ireland|Israel|Syria|Swaziland|Turks and Caicos Islands|Chad|Togo|Tajikistan|Tokelau|Turkmenistan|Tonga|Trinidad and Tobago|Tuvalu|Ukraine|Uganda|Uruguay|Uzbekistan|Saint Vincent and the Grenadines|Virgin Islands|Wallis and Futuna|Mayotte|Zambia|Zimbabwe|British Indian Ocean Territory|Iraq|Iceland|Jamaica|Jordan|Kenya|Kyrgyzstan|Cambodia|Kiribati|Saint Kitts and Nevis|North Korea|Cayman Islands|Kazakhstan|Lebanon|Saint Lucia|Liechtenstein|Sri Lanka|Liberia|Lesotho|Luxembourg|Latvia|Libya|Morocco|Monaco|Moldova|Montenegro|Madagascar|Marshall Islands|Macedonia|Mali|Mongolia|Northern Mariana Islands|Martinique|Mauritania|Montserrat|Malta|Mauritius|Maldives|Mozambique|Namibia|Niger|Norfolk Island|Nigeria|Nicaragua|Nepal|Nauru|Niue|Peru|French Polynesia|Pakistan|Poland|Saint Pierre and Miquelon|Pitcairn Islands|Puerto Rico|Palau|Paraguay|Qatar|Romania|Serbia|Rwanda|Seychelles|Sudan|Singapore|Saint Helena|Slovenia|Slovakia|San Marino|Senegal|Somalia|Suriname|Sao Tome and Principe|El Salvador|Armenia|Angola|Argentina|Australia|Azerbaijan|Bangladesh|Brunei|Brazil|Bahamas|Canada|Ivory Coast|Chile|China|Cuba|Cape Verde|Germany|Denmark|Ecuador|Estonia|Eritrea|Spain|Finland|Fiji|Falkland Islands|Faroe Islands|France|United Kingdom|Greenland|Equatorial Guinea|Greece|Croatia|Haiti|Indonesia|India|Thailand|Timor-Leste|Tunisia|Turkey|Tanzania|United States|Venezuela|Vietnam|Vanuatu|Samoa|Yemen|South Africa|Iran|Italy|Japan|Comoros|South Korea|Kuwait|Lithuania|Myanmar|Malawi|Mexico|Malaysia|New Caledonia|Netherlands|Norway|New Zealand|Oman|Panama|Papua New Guinea|Philippines|Portugal|Russia|Saudi Arabia|Solomon Islands|Sweden|Jan Mayen|Sierra Leone|U.S.|Taiwan)\b/g;
  ...
  sendToApi = {locationObj, countryMatchOjb, areaMatchOjb, distanceMatchOjb, stateOjb}
	...
	//var urimerge = "http://visualization.ischool.uw.edu:5000/todo/api/v1.0/merge/"; // test uri
  var urimerge = "http://flask-env.82nggiyg3i.us-west-2.elasticbeanstalk.com/todo/api/v1.0/merge/"; // aws elastic beanstalk

  // Send our entire tagged document to our API
	doRequest(urimerge, JSON.stringify(sendToApi), function(err, response) { 
    if (err) {
      console.log('Error: ' + err);
    } else {
      responseArray = JSON.parse(response);
      final_content = $('body').html();  // re-inject the entire document
      ...
      // For each tagged country, tag it
      ...
      // For each tagged area measurement, tag it
      var myindArea = 0;
      ...
      // For each tagged distance measurement, tag it
      var myindDistance = 0;
  ...
  console.log($('#loadingLayer-altas').remove()) // remove the transparent overlay on the page
  create_tooltip(); // create each of the tooltips associated to each highlighted term
} // highlight end
...
// Width + Height of the parent container
var largeWidth = 280;
var largeHeight = 280;
var padding = 5;
...
var baseid;
var targetid;
var zoomToBoxScale = d3.scale.log().domain([17098242, 50]).range([100, 600]); // set the scales + zooming of the topojson charts
// land colors
var colors = ["#EEEAE2", "#FFD300"]


// define state, with default values, but these get changed
var state = {
  latLon: [{
    lat: 6.52865,
    lon: 20.3586336
...
function drawfromarticle(matchObject, center_lat, center_lon, userarea, this_country_lat, this_country_lon, this_country_area, scale, topoid, topoidState) {
  ...
  areas[1] = areas[1] * 0.6; // scales our second, smaller area so that the animation is more dramatic
